...
Fresh installs can be bootstrapped with a broad English starter catalog (compiled from common cooking-blog usage): **34 units** with metric/imperial conversion factors and **319 ingredients** grouped by category, each with **per-100 g nutrition facts**.
...
- **Nutrition included**: every seeded ingredient gets reference values for kcal, fat, saturated fat, carbs, sugars, protein, fiber and salt (per 100 g) — a starting point to refine in the Ingredients screen.
...
Data lives in `includes/data/seed.php` (catalog + translations) and `includes/data/nutrition.php`; logic in `includes/Seeder.php`. Polylang translations are written via `PLL_MO`, WPML via the String Translation API.
...
- **Starter catalog seeder**: manual Settings → Catalog action to fill empty units/ingredients tables with a broad English starter set (34 units, 319 ingredients) — including per-100 g nutrition facts and IT/FR/DE/ES translations pre-loaded into WPML/Polylang for the site's configured languages; never touches a populated catalog.
